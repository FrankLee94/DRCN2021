...
class Embedding:
	def __init__(self):
		...
		self.CPU_CAPA = 2000		# 单节点CPU容量
		self.RAM_CAPA = 2000		# 单节点RAM容量
		self.WAVE_NUM = 96			# 链路波长数
		self.WAVE_CAPA = 200		# 单波长100G		
		...
		self.traff_df = pd.read_excel('./traffic_5.xlsx')
	...
	# 业务接入，链路负载增加
	# info_best_paths: 多条路的附带信息，每条路信息为is_new_lp, old_lp_id, wave
	...
	# info_best_paths，包含有每一条path里面对应的信息，is_new_lp, old_lp_id, wave
		...
		for i in range(len(all_best_paths)):
			...
			if len(path) > 0:		# 非空path
				if info_best_paths[i][0]:		# 使用新的lp
					...
					bd = int(row['bd'])
					self.lp_No += 1
					self.lp_idx[self.lp_No] = copy.deepcopy([self.lp_No, s, d, bd, all_best_paths[i]])
					for j in range(len(path) - 1):
						s = path[j]
						d = path[j+1]					
					...
					d = path[-1]
					bd = int(row['bd'])
					old_lp_id = info_best_paths[i][1]
					self.lp_idx[(s, d)][old_lp_id][4] += bd
			else:
				pass
	...
	def bip_judge_path(self, row, path):
		...
		return is_path, is_new_lp, old_lp_id, wave, min_bd

	# k条最短路径里面找一条，包含最大剩余带宽的
	# 同时返回是否使用新lp，如果使用的是旧lp，旧lp的id是什么
	def bip_find_best_path(self, row, k_paths):
		...
		best_path = []
		for path in k_paths:
			is_path, is_new_lp, old_lp_id, wave, min_bd = self.bip_judge_path(row, path)
			if is_path:		# 该链路是可行的，找包含最大剩余带宽的
				if min_bd < all_min_bd:
					best_path = path
					is_new_lp_best = is_new_lp
					old_lp_id_best = old_lp_id
					wave_best = wave
					all_min_bd = min_bd
			else:
				pass
		return best_path, is_new_lp_best, old_lp_id_best, wave_best

	# 每两个功能之间的单条可用链路: best_path
	# 找到的所有链路集合: all_best_paths
	# info_best_paths: 储存每一条best_path附带的信息，包括is_new_lp, old_lp_id, wave
	def bip_find_paths(self, row, match):
		...
		for i in range(0, int(row['vnf_num']) + 1):
			...
			# k条最短路中的最好路
			best_path, is_new_lp, old_lp_id, wave = self.bip_find_best_path(row, k_paths)
			if len(best_path) == 0:			# 找不到合适的路
				all_best_paths = []
				info_best_paths = []
				return all_best_paths, info_best_paths		# 直接返回空
			all_best_paths.append(copy.deepcopy(best_path))
			info_best_paths.append(copy.deepcopy([is_new_lp, old_lp_id, wave]))
