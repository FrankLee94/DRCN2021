...
class Embedding:
	def __init__(self):
		self.FG_NUM = 1000			# 总业务数
		self.ERLANG = 800			# 网络稳定时存在的业务数
		self.already_access = 0		# 已经接入的数量
		...
		self.CPU_CAPA = 12*400		# 单节点CPU容量
		self.RAM_CAPA = 64*400		# 单节点RAM容量
	...
	def bip_sig_req_arriv(self, row):
		if self.already_access < self.ERLANG:
			is_access, match, all_best_paths, info_best_paths = self.bip_is_req_access(row)
		else:
			is_access, match, all_best_paths, info_best_paths = [False, {}, [], []]
	...
	def coll_info_fail_node(self):
		for req_no in range(self.FG_NUM):
			is_access, match, all_best_paths, info_best_paths = self.vm_idx[req_no]
			if is_access:			# 接入的请求
				s = int(self.traff_df.loc[req_no]['s'])
				d = int(self.traff_df.loc[req_no]['d'])
				bd = int(self.traff_df.loc[req_no]['bd'])
				...
				if self.fail_node in match.keys():
					...
					# 失效节点不是源宿节点，只需要恢复一个网络功能
						...
						vnf_order = eval(self.traff_df.loc[req_no]['vnf_order'])
	...
	def cal_all_fail_flows(self):
		...
		print('fg need recover: ' + str(self.fg_need_reco))
	...
	def statistics_gne(self):
		...
		print('gne info')
		print('Erlang: ' + str(self.ERLANG))
		print('recover success with vnf: ' + str(self.success_count_gne[0]))
		print('recover success without vnf: ' + str(self.success_count_gne[1]))
		print('recover success of gne: ' + str(over_all_success))
		print('recover rate of gne: ' + str(round(over_all_success * 100 / self.fg_need_reco, 2)) + '%')
		print('recover rate with all: ' + str(round(over_all_success * 100 / (self.fg_need_reco + len(self.fail_fg_forever)), 2)) + '%')
		print('fail recover because of node capacity: ' + str(self.fail_reason_gne[0]))
		print('fail recover because of path unavailability: ' + str(self.fail_reason_gne[1]))
	...
	def cal_var(self):
		...
		for i in range(self.NODE_NUM):
			if i == self.fail_node:
				continue		# 失效节点不用考虑
			var_matrix[i] = np.var(res_matrix[i])
	...
	def statistics_var(self):
		...
		print('var info')
		print('Erlang: ' + str(self.ERLANG))
		print('recover success with vnf: ' + str(self.success_count_var[0]))
		print('recover success without vnf: ' + str(self.success_count_var[1]))
		print('recover success of var: ' + str(over_all_success))
		print('recover rate of var: ' + str(round(over_all_success * 100 / self.fg_need_reco, 2)) + '%')
		print('recover rate with all: ' + str(round(over_all_success * 100 / (self.fg_need_reco + len(self.fail_fg_forever)), 2)) + '%')
		print('fail recover because of node capacity: ' + str(self.fail_reason_var[0]))
		print('fail recover because of path unavailability: ' + str(self.fail_reason_var[1]))
